...
app.post('/cases/new', requireLogin, wrap(async (req, res) => {
  ...
  if (!title || !(amount > 0) || !otherEmail || !['owed', 'owe'].includes(role)) return res.render('new-case', { error: 'Please fill in every field with valid values.', currencies: CURRENCIES, selectedCurrency: curOf(req.body.currency).code, paymentsEnabled: PAYMENTS_ENABLED });
...
app.get('/cases/:id', requireLogin, wrap(async (req, res) => {
  ...
  res.render('case', { c, role, status, inviteUrl, pay, cur: curOf(c.currency) });
}));
