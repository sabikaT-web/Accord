...
app.get('/cases/:id', requireLogin, wrap(async (req, res) => {
  ...
  const myDetailsDone = role === 'claim' ? !!c.claim_address : !!c.resp_address;
  const bothDetailsDone = !!(c.claim_address && c.resp_address);
...
app.post('/cases/:id/party-details', requireLogin, wrap(async (req, res) => {
  ...
  const company = (req.body.company || '').trim().slice(0, 200);
  const address = (req.body.address || '').trim().slice(0, 500);
  await db.setPartyDetails(role, company || null, address || null, c.id);
  await db.addEvent(c.id, 'details', (role === 'claim' ? 'Claimant' : 'Respondent') + ' added their details for the agreement');
  const fresh = await db.caseById(c.id);
  const bothIn = !!(fresh.claim_address && fresh.resp_address);
  res.json({ ok: true, bothIn, signUrl: '/cases/' + c.id + '/sign' });
}));

// Hand off to e-signature. Set DOCUSIGN_URL to your DocuSign signing/powerform
// link; until then this falls back to the printable agreement.
app.get('/cases/:id/sign', requireLogin, wrap(async (req, res) => {
  const c = await db.caseById(Number(req.params.id));
  if (!c) return res.status(404).render('message', { title: 'Not found', body: 'That case does not exist.' });
  const role = roleOf(c, req.session.userId);
  if (!role && !res.locals.isAdmin) return res.status(403).render('message', { title: 'No access', body: 'You are not a party to this case.' });
  if (c.status !== 'settled') return res.status(400).render('message', { title: 'Not yet agreed', body: 'Both sides must settle before the agreement can be signed.' });
  if (!c.agreement_sent) await db.markAgreementSent(c.id);
  const base = process.env.DOCUSIGN_URL;
  if (base) {
    const sep = base.includes('?') ? '&' : '?';
    return res.redirect(base + sep + 'caseRef=MB-' + c.id);
  }
  res.render('message', {
    title: 'Agreement on its way',
    body: 'Your settlement agreement is being prepared for signature and will be emailed to both parties. In the meantime you can view and print it from the case page.'
  });
}));
